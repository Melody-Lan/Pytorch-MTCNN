...
class FaceDataset(Dataset):
    ...
    def __init__(self, path):
        self.path = path
        self.dataset = []
        self.dataset.extend(open(os.path.join(path, "positive.txt")).readline())
        self.dataset.extend(open(os.path.join(path, "negative.txt")).readline())
        self.dataset.extend(open(os.path.join(path, "part.txt")).readline())
...
if __name__ == '__main__':
    dataset = FaceDataset(r"D:\celeba\48")
    print(dataset[0])