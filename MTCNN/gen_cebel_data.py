...
for face_size in [48]:
    ...
    for path_dir in [positive_img_dir, negative_img_dir, part_img_dir]:
        ...
        for i, line in enumerate(open(anno_src)):
            ...
            try:
                ...
                with Image.open(image_file) as img:
                    ...
                    # x1 = float(str[1].strip())
                    # y1 = float(str[2].strip())
                    # w = float(str[3].strip())
                    # h = float(str[4].strip())
                    # x2 = float(x1 + w)
                    # y2 = float(x2 + h)
                    ...
                    cy = y1 + h / 2
                    for _ in range(5):
                        _w = np.random.randint(-w * 0.3, w * 0.3)
                        _h = np.random.randint(-h * 0.2, h * 0.2)
                        cx_ = cx + _w
                        cy_ = cy + _h

                        # 让人脸形成正方形，并且让坐标也有少许的偏离
                        side_len = np.random.randint(int(min(w, h)), np.ceil(1.03 * max(w, h)))
                        ...
                        if iou > 0.55:# 正样本
                            ...
                            face_resize.save(os.path.join(positive_img_dir, "{0}.jpg".format(positive_count)))
                            positive_count += 1
                        elif iou > 0.4:  # 部分样本
